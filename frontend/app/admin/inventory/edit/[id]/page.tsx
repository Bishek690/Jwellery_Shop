...
export default function EditProductPage() {
  ...
  const handleInputChange = (field: keyof EditProductFormData, value: string | boolean | File) => {
  ...
  const validateForm = () => {
    ...
    const missing = required.filter(field => !formData[field as keyof EditProductFormData])
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs sm:text-sm text-gray-600 mt-1 hidden sm:block">Update product information in your inventory</p>
          </div>
        </div>
      </div>
                  ...
                  <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    ...
                    <div className="space-y-1">
                      <div className="text-sm text-green-700">
                        Regular Profit: ₹{(parseFloat(formData.price) - parseFloat(formData.cost)).toFixed(2)} 
                        ({((parseFloat(formData.price) - parseFloat(formData.cost)) / parseFloat(formData.price) * 100).toFixed(1)}%)
                      </div>
                      {formData.discountPrice && formData.discountPrice.trim() !== "" && parseFloat(formData.discountPrice) > 0 && (
                        <div className="text-sm text-green-600 font-medium">
                          Discounted Profit: ₹{(parseFloat(formData.discountPrice) - parseFloat(formData.cost)).toFixed(2)} 
